...
def vector():
	for u in users:
		...
		users[u].u_mean = users[u].u_count/53388
...
def tag_similarity(u1,u2):
	...
	for t in users[u1].u_tags:
		if(t in users[u2].u_tags):
			...
			den1 += (users[u1].u_t_vector[t])*(users[u1].u_t_vector[t])
			den2 += (users[u2].u_t_vector[t])*(users[u2].u_t_vector[t])
...
def frs():
	counter = 1
	for u1 in users:
		print(str(counter) + " / " + str(len(users)))
		for u2 in users:
			if(u1 != u2):
				user1 = users[u1]
				user2 = users[u2]
				sim = tag_similarity(u1,u2)
				user_interest1 = user_interest(u1,u2)
				user_interest2 = user_interest(u2,u1)
				if(sim>alpha and (user_interest1>beta or user_interest2>beta)):
					S.append((u1,u2))
		counter += 1
